...
class TestForwardFeatureSelection:
    ...
    def test_compute_model_performances(self, mocker, model_type):
        ...
        if model_type == "classification":
            ...
            patch_fct = (
                "cobra.model_building.forward_selection.LinearRegressionModel.evaluate"
            )

        mocker.patch(patch_fct, mock_evaluate)

        actual = fw_selection.compute_model_performances(
            data, "target", splits=["train", "selection"], metric=None
        )
    ...
    @pytest.mark.parametrize(
        "model_type, max_predictors, expectation",
        [
    ...
    def test_fit(self, mocker, model_type, max_predictors: int, expectation):
        ...
        ordered_output_list = forced_predictors_list + [
            pred for pred in predictors_list if pred not in forced_predictors_list
        ]

        fw_selection = ForwardFeatureSelection(
            model_type=model_type, max_predictors=max_predictors
        )

        def mock_train_model(self, train_data, target_column_name, predictors):
            return mock_model(predictors, model_type=model_type)

        def mock_forward_selection(
            self, train_data, target_column_name, predictors, forced_predictors
        ):
            n_models = min(max_predictors, len(predictors) + len(forced_predictors))

            return [
                mock_model(ordered_output_list[: i + 1], model_type=model_type)
        ...
        mocker.patch(
            "cobra.model_building.ForwardFeatureSelection._forward_selection",
        ...
        with expectation:
            ...
            actual = [model.predictors for model in fw_selection._fitted_models]
    ...
    def test_forward_selection(self, mocker, model_type, max_predictors: int):
        ...
        predictors = [pred for pred in predictors_list if pred not in forced_predictors]

        ordered_output_list = forced_predictors + predictors

        def mock_find_next_best_model(
            self,
            train_data,
            target_column_name,
            ...
            return mock_model(
                current_predictors + candidate_predictors[0:1], model_type=model_type
            )

        mocker.patch(
            ("cobra.model_building.ForwardFeatureSelection." "_find_next_best_model"),
            mock_find_next_best_model,
        )
