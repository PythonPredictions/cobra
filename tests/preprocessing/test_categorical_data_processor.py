...
class TestCategoricalDataProcessor:
    def test_attributes_to_dict(self):
        ...
        processor._cleaned_categories_by_column = {"variable": set(cleaned_categories)}
    ...
    def test_set_attributes_from_dict(self, attribute):
        ...
        params = {
            ...
            "scale_contingency_table": True,
            "forced_categories": {},
            "_cleaned_categories_by_column": {"variable": cleaned_categories},
        }
    ...
    def test_replace_missings(self):
        ...
        actual = CategoricalDataProcessor._replace_missings(data, ["variable"])

        pd.testing.assert_frame_equal(actual, expected)

    @pytest.mark.parametrize(
        "cleaned_categories, expected",
        [
            ({"c1", "c2"}, pd.Series(data=["c1", "c2", "Other", "Other"])),
            ({"c1", "c2", "c3", "c4"}, pd.Series(data=["c1", "c2", "c3", "c4"])),
        ],
    )
    ...
    def test_all_cats_not_significant(self):
        ...
        expected = pd.DataFrame(
            e, columns=["categorical_var", "target", "categorical_var_processed"]
        )
    ...
    def test_regroup_name(self):
        ...
        expected["categorical_var_processed"] = expected[
            "categorical_var_processed"
        ].astype("category")

        categorical_data_processor = CategoricalDataProcessor(
            category_size_threshold=0, regroup_name="OTH", p_value_threshold=0.05
        )
